...
#PSAP_PATH=/home/local/ARCS/nz2274/Application/ #INSERT PATH TO PSAP DIRECTORY HERE eg. /scratch/dclab/P
...
#exit;  
	echo "filter vcf to $VCF.filter.vcf"
	bn_vcf=$(basename $VCF)
	if [ ! -e ${bn_vcf}.filter.vcf ];then
...
if [ ! -e $curpath/annotated/${OUTFILE}.avinput.hg19_multianno.txt  ] || [ $ln  -lt 3 ] ; then    
	...
	perl ${ANNOVAR_PATH}convert2annovar.pl -format vcf4old  $curpath/annotated/$VCF.filter.vcf -outfile $curpath/annotated/${OUTFILE}.avinput -includeinfo
